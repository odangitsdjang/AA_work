...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    ...
    FROM routes AS a
    JOIN routes AS b ON (a.company = b.company AND b.num = a.num)
...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN stops stopa ON (stopa.id = a.stop_id)
    JOIN stops stopb ON (stopb.id = b.stop_id)
...
def craiglockhart_to_sighthill
  ...
  #  routes start
  # ^ same start same num
  # routes mid point from start
  # ^ and bottom is same stop id but different company
  # routes  midpoint to finish
  # ^ and bottom same company and same num
  # routes   finish

  execute(<<-SQL)
    SELECT DISTINCT a.num, a.company, midpoint.name, d.num, d.company
    FROM routes a
    JOIN routes b ON (a.company = b.company AND a.num = b.num)
    JOIN stops midpoint ON (midpoint.id = b.stop_id)
    JOIN routes c ON  midpoint.id = c.stop_id
    JOIN routes d ON (c.company = d.company AND c.num = d.num)
    WHERE a.stop_id = (
      SELECT stops.id
      FROM stops
